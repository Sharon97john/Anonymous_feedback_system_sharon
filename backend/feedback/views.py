    ...
    if request.method == "POST":
        ...
        if(answer.objects.filter(feedback_id=post_data["feedback_id"])):
            question_list = question.objects.filter(
                feedback_id=post_data['feedback_id']).filter(isOption=True)
            ...
            for quest in question_list:
                options = OptionsSerializer(optionsTable.objects.filter(
                    question_id=quest["id"]), many=True)
                ...
                for option in options:
                    count = answer.objects.filter(feedback_id=post_data["feedback_id"]).filter(
                        question_id=quest["id"]).filter(description=str(option["id"])).count()
